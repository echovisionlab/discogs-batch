...
class MappedValueValidatorTest {
    ...
    void shouldReportForNullValue(ArgType argType) {
        ...
        argBuilder.append(argType.getGlobalName())
                .append("=");
    ...
    void shouldReportNonSupportedMultipleValues(ArgType argType) {
        ...
        if (max < 3 || min > 3) {
            if (min == max) {
                assertThat(result.getIssues().get(0)).isEqualTo(
                        argType.getGlobalName() + " expected " + min + " items but got 3 item");
            } else {
                assertThat(result.getIssues().get(0)).isEqualTo(
                        argType.getGlobalName() + " expected " + min + " to " + max + " items but got 3 item");
            }
        }
    }
    ...
    void shouldReportUnsupportedValueType(ArgType argType) {
        ...
        String expectedMsg = "invalid type for " +
