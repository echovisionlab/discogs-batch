...
class DataSourceArgumentValidatorTest {

    private final DataSourceArgumentValidator validator =
            new DataSourceArgumentValidator();
    ...
    void shouldReportMalformedJdbcUrl() {
        String response = "invalid url format. expected: jdbc:mysql://{address}:{port}/schema_name{?options}";
    ...
    void shouldReportAsBlankIfEverythingIsPresent() {
        String[] arg = new String[]{"user=hello", "pass=pass", "url=jdbc:mysql://localhost:3306/something"};
        ValidationResult result = validator.validate(new DefaultApplicationArguments(arg));
