...
/**
 * Validate if argument is required to be mapped or not. If one is not supposed to have a value mapped,
 * it will report with {@link ArgumentValidator}. The same applies to the one that is supposed to be mapped.
 */
public class MappedValueValidator implements ArgumentValidator {
    ...
    private static final String INVALID_SIZE_RANGE_MSG = "%s expected value count of %d to %d but got %d item";

    /**
     * Combined validation for either having mapped but not supposed to be mapped, or the opposite case.
     *
    ...
    private Map<ArgType, List<String>> collectArguments(ApplicationArguments args) {
        ...
        for (String nonOptionArg : args.getNonOptionArgs()) {
            ...
            List<String> values = Arrays.stream(nonOptionArg.substring(delimiterIndex + 1).split(","))
    ...
    private ValidationResult validateArgumentCount(Map<ArgType, List<String>> argMap) {
        ...
        for (ArgType type : argMap.keySet()) {
            ...
            argMap.get(type).forEach(value -> {
                if (MULTI_VALUE_PATTERN.matcher(value).matches()) {
                    List<String> multiValue = List.of(value.split(",")).stream()
