...
public class DefaultArgumentHandler implements ArgumentHandler {
    ...
    /**
     * Splits multiple values for OPTIONS into individual entries.
     * It will pass through the NonOptional arguments even if it has several arguments with ',' delimiters.
     */
    private final Function<String, List<String>> splitMultiValues = arg -> {
    ...
    /**
     * Resolves url formatting if the entry exists.
     * Then passes everything into a validator to perform actual validation.
     *
    ...
    public ApplicationArguments normalizeArguments(ApplicationArguments args) {
        List<String> normalizedResult = Arrays.stream(args.getSourceArgs())
