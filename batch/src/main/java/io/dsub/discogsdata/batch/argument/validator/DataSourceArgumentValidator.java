...
/**
 * A validator to check necessary information to perform initialization of {@link javax.sql.DataSource}.
 * The validator will basically require three arguments: user, pass and url.
 * First it will check the existence and duplication of each arguments.
 * Next it will decide if additional validation can be performed.
 * Finally, it will resolve the url format by pattern to check if something is wrong.
 * <p>
 * Results from the first step will be accumulated then returned if any issue exists.
 * Final validation for url will simply validate the format.
 */
public class DataSourceArgumentValidator implements ArgumentValidator {
    ...
    private static final String urlPattern =
            "^jdbc:mysql://" + // jdbc header
                    CHARACTERS + // address
                    ":" + // to designate address port
                    PORT_RANGE +  // port range of (1-99999) while not 03 or 010
                    "/" + // directory support
                    CHARACTERS + // schema name
                    "(\\?" + QUERY_PART + "=" + QUERY_PART + // opening queries
    ...
    private ValidationResult checkUrl(ApplicationArguments args, ValidationResult validationResult) {
        String jdbcConnectionString = args.getNonOptionArgs().stream()
    ...
    private ValidationResult checkIfExists(ApplicationArguments args, ValidationResult result) {
        ...
        List<ArgType> foundTypes = args.getNonOptionArgs().stream()
                .map(arg -> arg.split("=")[0])
        ...
        List<String> missingArgumentIssues = REQUIRED_TYPES.stream()
    ...
    private ValidationResult checkDuplicates(ApplicationArguments args, ValidationResult result) {
        ...
        Map<ArgType, List<String>> targetArguments = args.getNonOptionArgs().stream()
                .filter(argString -> REQUIRED_TYPES.stream()
                        .anyMatch(argType -> argType.getSynonyms().contains(argString.split("=")[0])))
                .collect(Collectors.groupingBy(argString -> ArgType.getTypeOf(argString.split("=")[0])));

        List<String> issuesList = targetArguments.entrySet().stream()
