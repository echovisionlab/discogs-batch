...
class DiscogsDumpRepositoryUnitTest {
  ...
  void whenCountItemsAfter__ShouldReturnValidCount() {
    ...
    int expected = (int) DUMP_LIST.stream()
        .filter(
            dump -> dump.getLastModifiedAt().isEqual(ld) || dump.getLastModifiedAt().isAfter(ld))
        .count();
    // then
  ...
  void countItemsBefore__ShouldReturnValidCount() {
    ...
    int expected = (int) DUMP_LIST.stream()
        .filter(
            dump -> dump.getLastModifiedAt().isEqual(ld) || dump.getLastModifiedAt().isBefore(ld))
        .count();
    // then
    assertThat(count).isEqualTo(expected);
  }
  ...
  void countItemsBetween__ShouldReturnValidCount() {
    ...
    int expected = (int) DUMP_LIST.stream()
        ...
        .filter(dump -> dump.getLastModifiedAt().isEqual(start) || dump.getLastModifiedAt()
            .isAfter(start))
        .count();
    // then
    assertThat(count).isEqualTo(expected);
  ...
  @ParameterizedTest
  @MethodSource("io.dsub.discogs.batch.TestArguments#getLocalDateTimes")
  void whenFindTopByTypeAndLastModifiedAtBetween__ShouldOnlyPresentItemsWithGivenTimeframe(
      LocalDate ldt) {
    for (EntityType type : EntityType.values()) {
      DiscogsDump dump = repository
          .findTopByTypeAndLastModifiedAtBetween(type, ldt, ldt.plusMonths(1));
      if (dump != null) {
  ...
  void whenFindByTypeAndLastModifiedAtBetween__ShouldReturnValidItems(LocalDate ldt) {
    for (EntityType type : EntityType.values()) {
      List<DiscogsDump> result = repository
          .findByTypeAndLastModifiedAtBetween(type, ldt, ldt.plusMonths(1));

      result
          .forEach(dump -> assertThat(dump.getLastModifiedAt()).isBetween(ldt, ldt.plusMonths(1)));
    }
  }
  ...
  void whenFindTopByTypeAndLastModifiedAtBetween__ShouldReturnMostRecentItem() {
    ...
    // when
    DiscogsDump dump = repository
        .findTopByTypeAndLastModifiedAtBetween(EntityType.ARTIST, ld, ld.plusMonths(1));
