...
public class ReflectionUtil {
    ...
    public static List<Field> getDeclaredFields(Class<?> target, Predicate<Field> condition) {
        return getDeclaredFields(target).stream()
                .filter(condition)
                .collect(Collectors.toList());
    }
    ...
    public static <T> T invokeNoArgConstructor(Class<T> clazz) {
        try {
            ...
            throw new InvalidArgumentException(clazz.getSimpleName() + " does not have no-arg constructor");
        } catch (Throwable e) {
