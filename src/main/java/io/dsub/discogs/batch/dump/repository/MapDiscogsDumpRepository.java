...
public class MapDiscogsDumpRepository implements DiscogsDumpRepository, InitializingBean {
    ...
    public DiscogsDump findByETag(String ETag) {
        return cache.stream()
                .filter(dump -> dump.getETag().equals(ETag))
                .findFirst()
                .orElse(null);
    }
    ...
    public int countItemsAfter(LocalDate start) {
        return (int) cache.stream()
                .filter(dump -> isDumpEqualOrAfter(dump, start))
                .count();
    }
    ...
    public int countItemsBefore(LocalDate end) {
        return (int) cache.stream()
                .filter(dump -> isDumpEqualOrBefore(dump, end))
                .count();
    }
    ...
    @Override
    public List<DiscogsDump> findByTypeAndLastModifiedAtBetween(EntityType type, LocalDate start, LocalDate end) {
        return cache.stream()
    ...
    @Override
    public DiscogsDump findTopByTypeAndLastModifiedAtBetween(EntityType type, LocalDate start, LocalDate end) {
        return cache.stream()
                .filter(dump -> dump.getType().equals(type))
                .filter(dump -> isDumpEqualOrBetween(dump, start, end))
